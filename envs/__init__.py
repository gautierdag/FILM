...
class VecPyTorch:
    ...
    def va_interact(
        self, action, interact_mask=None, smooth_nav=True, mask_px_sample=1, debug=False
    ):
        (
            obs,
            rew,
...
def construct_envs_alfred(args):
    ...
    files = json.load(open("alfred_data_small/splits/oct21.json"))[args.eval_split][
    ...
    envs = VectorEnv(
        ...
        env_fn_args=tuple(
            tuple(zip(args_list, scene_names_list, range(args.num_processes)))
        ),
    )
