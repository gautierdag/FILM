...
from models.instructions_processed_LP.ALFRED_task_helper import (
    get_list_of_highlevel_actions,
    get_arguments,
    get_arguments_test,
    read_test_dict,
)
from models.segmentation.segmentation_helper import SegmentationHelper
...
class Sem_Exp_Env_Agent_Thor(ThorEnvCode):
    ...
    def __init__(self, args, scene_names, rank):
        ...
        self.test_dict = read_test_dict(
            self.args.test, self.args.appended, "unseen" in self.args.eval_split
        )

        # Segmentation
        self.seg = SegmentationHelper(self)

        # Depth

    def load_traj(self, scene_name):
        json_dir = (
            "alfred_data_all/json_2.1.0/"
            + scene_name["task"]
            + "/pp/ann_"
            + str(scene_name["repeat_idx"])
            + ".json"
        )
    ...
    def load_initial_scene(self):
        ...
        episode_no = (
            self.args.from_idx
            + self.scene_pointer * self.args.num_processes
            + self.rank
        )
        ...
        try:
            ...
            if task_type == "look_at_obj_in_light":
                ...
                self.cat_equate_dict[
                    "DeskLamp"
                ] = "FloorLamp"  # if DeskLamp is found, consider it as FloorLamp

            if sliced:
                self.total_cat2idx["ButterKnife"] = self.total_cat2idx["Knife"]
                self.cat_equate_dict[
                    "ButterKnife"
                ] = "Knife"  # if DeskLamp is found, consider it as FloorLamp

            actions_dict = {
                "task_type": task_type,
                "list_of_actions": list_of_actions,
                "second_object": second_object,
                "total_cat2idx": self.total_cat2idx,
                "sliced": self.sliced,
            }
            ...
            self.print_log(
                "Scene pointers exceeded the number of all scenes, for env rank",
                self.rank,
            )
    ...
    def va_interact_new(self, action, mask=None):
        ...
        if not (self.args.approx_last_action_success):
            ...
            success = CH._get_approximate_success(
                self.prev_rgb, self.event.frame, action
            )
            self.last_success = success
    ...
    def reset_goal(self, truefalse, goal_name, consecutive_interaction):
        if truefalse == True:
            ...
            if "Sliced" in goal_name:
                self.cur_goal_sliced = self.total_cat2idx[
                    goal_name.replace("Sliced", "")
                ]
            else:
            ...
            if (
                abs(int(self.camera_horizon) - 45) > 5
                and consecutive_interaction is None
            ):
                (
                    obs,
                    rew,
    ...
    def consecutive_interaction(self, interaction, target_instance):
        if interaction == "PutObject" and self.last_action_ogn == "OpenObject":
            ...
            self.interaction_mask = self.seg.sem_seg_get_instance_mask_from_obj_type(
                target_instance
            )
        else:
            self.interaction_mask = self.seg.get_instance_mask_from_obj_type(
                target_instance
            )
        ...
        if self.last_action_ogn == "PickupObject" and success:
            ...
            if self.args.use_sem_seg:
                self.picked_up_mask = (
                    self.seg.sem_seg_get_instance_mask_from_obj_type_largest_only(
                        self.goal_name
                    )
                )
            else:
                self.picked_up_mask = self.seg.get_instance_mask_from_obj_type_largest(
                    self.goal_name
                )
            ...
            self.put_rgb_mask = self.seg.H.diff_two_frames(
                self.prev_rgb, self.event.frame
            )
            self.picked_up_mask = None
    ...
    def side_step(self, step_dir, cur_start_o, cur_start, traversible):
        ...
        # Move ahead
            ...
            if (
                hasattr(self.args, "check_before_sidestep")
                and self.args.check_before_sidestep
            ):
                self.print_log("checking consec moving!")
                xy = CH._which_direction(cur_start_o)
                whether_move = CH._check_five_pixels_ahead_map_pred_for_moving(
                    self.args, traversible, cur_start, xy
                )
            else:
                whether_move = True
            if not (whether_move):
    ...
    def move_until_visible(self):
        if (
            not (self.rotate_aftersidestep is None)
    ...
    def plan_act_and_preprocess(self, planner_inputs, goal_spotted):
        ...
        if planner_inputs["wait"]:
            ...
            return (
                ...
                False,
                {
                    ...
                    "move_until_visible_cycled": self.mvb_which_cycle != 0
                    and self.mvb_num_in_cycle == 0,
                    "delete_lamp": False,
                    "fails_cur": 0,
                },
            )
        ...
        if self.side_step_order in [1, 2]:
            ...
            if self.args.use_sem_seg:
                self.print_log(
                    "obj type for mask is :", self.goal_idx2cat[self.goal_idx]
                )
                self.interaction_mask = (
                    self.seg.sem_seg_get_instance_mask_from_obj_type(
                        self.goal_idx2cat[self.goal_idx]
                    )
                )
            else:
                self.interaction_mask = self.seg.get_instance_mask_from_obj_type(
            ...
            if prev_side_step_order == 2:
                ...
                if not (pointer in self.caution_pointers):
                    ...
                    self.execute_interaction = (
                        goal_spotted
                        and visible
                        and self.prev_number_action == 0
                        and whether_center
                    )
                    if opp_side_step:
                        self.execute_interaction = (
                            goal_spotted and visible and self.prev_number_action == 0
                        )
            ...
            planner_inputs["consecutive_interaction"] == None
            and self.execute_interaction
        ):
            ...
            if interaction == None:
                ...
                if self.last_action_ogn == "PickupObject" and goal_success:
                    ...
                    self.picked_up_cat = self.goal_idx
                    if self.args.use_sem_seg:
                        self.picked_up_mask = self.seg.sem_seg_get_instance_mask_from_obj_type_largest_only(
                            self.goal_name
                        )
                    else:
                        self.picked_up_mask = (
                            self.seg.get_instance_mask_from_obj_type_largest(
                                self.goal_name
                            )
                        )
                    obs_temp, seg_print = self.preprocess_obs_success(success, obs)
            ...
            repeated_rotation = (
                (
                    not (self.last_three_sidesteps[0] is None)
                    and not (self.last_three_sidesteps[1] is None)
                )
                and "Rotate" in self.last_three_sidesteps[1]
                and self.prev_sidestep_success == False
            )

            if self.prev_number_action == 0:  # stop outputted now or before
                if (
                    self.args.approx_error_message
                    and not (self.last_success)
                ...
                elif not (self.args.no_rotate_sidestep) and (
                    not (self.last_three_sidesteps[0] is None)
                    and self.prev_sidestep_success == False
                ):
                ...
                elif self.is_visible_from_mask(
                    self.interaction_mask
                    ...
                    if (
                        ...
                        if step_dir == None:
                            ...
                            (
                                ...
                                opp_step_dir, cur_start_o, cur_start, traversible
                            )
                        ...
                        if not (step_dir is None):
                            (
                                ...
                                step_dir, cur_start_o, cur_start, traversible
                            )
                        else:
                            (
                                obs,
                    ...
                    if self.args.debug_local:
                        self.print_log(
                            "last three side stepped ", self.last_three_sidesteps
                        )
                    ...
                    if self.args.debug_local:
                        ...
                        self.print_log(
                            "move until visible order is ",
                            self.move_until_visible_order,
                        )
                    self.update_loc(planner_inputs)
                    (
                        obs,
                        ...
                        target_instance,
                        err,
                    ) = self.move_until_visible()
                    self.mvb_num_in_cycle += 1
                    if self.mvb_num_in_cycle == 12:
                        self.print_log(
                            "Went through one cycle of move until visible, step num is ",
                            self.steps_taken,
                        )
            ...
            if self.args.stricter_visibility <= 1.5:
                if (
                    self.is_visible_from_mask(
                        self.interaction_mask,
                        stricer_visibility_dist=self.args.stricter_visibility,
                    )
            ...
            if dist < col_threshold:  # Collision
                ...
                for i in range(length):
                    for j in range(width):
                        ...
                        r, c = wy, wx
                        r, c = int(round(r * 100 / args.map_resolution)), int(
                            round(c * 100 / args.map_resolution)
                        )
    ...
    def _get_stg(
            ...
            if goal_found or self.args.use_sem_policy:
                ...
                if self.dilation_deg == d_threshold:
                    ...
                    random_goal_idx = np.random.choice(
                        len(where_connected[0])
                    )  # choose a random goal among explored area
                    random_goal_ij = (
                        where_connected[0][random_goal_idx],
                        where_connected[1][random_goal_idx],
                    )
                ...
                if self.args.debug_local:
                    self.print_log(
                        "Goal in wall, or goal in obstruction althuogh not goal found"
                    )
                np.random.seed(self.steps_taken)
                random_goal_idx = np.random.choice(
                    len(where_connected[0])
        ...
        stg_x, stg_y, _, stop = planner.get_short_term_goal(
            state, found_goal=found_goal, decrease_stop_cond=decrease_stop_cond
        )
    ...
    def depth_pred_later(self, sem_seg_pred):
        rgb = cv2.cvtColor(
            self.event.frame.copy(), cv2.COLOR_RGB2BGR
        )  # shape (h, w, 3)
        rgb_image = torch.from_numpy(rgb).permute((2, 0, 1)).unsqueeze(0).half() / 255

        if abs(self.camera_horizon - 0) < 5:
            _, pred_depth = self.depth_pred_model_0.predict(
                rgb_image.to(device=self.depth_gpu).float()
            )

        else:
            _, pred_depth = self.depth_pred_model.predict(
                rgb_image.to(device=self.depth_gpu).float()
            )
        ...
        depth_img = pred_depth.get_trustworthy_depth(
            max_conf_int_width_prop=self.args.valts_trustworthy_prop,
        ...
        self.learned_depth_frame = (
            5 * 1 / 50 * np.argmax(self.learned_depth_frame, axis=0)
        )  # Now shape is (300,300)
        del pred_depth
        depth = depth_img
    ...
    def _preprocess_obs(self, obs):
        ...
        sem_seg_pred = self.seg.get_sem_pred(
            rgb.astype(np.uint8)
        )  # (300, 300, num_cat)
    ...
    def _visualize(self, inputs):
        ...
        color_palette2 = [int(x * 255.0) for x in color_palette2]
    ...
    def evaluate(self):
        ...
        s_spl = (1 if goal_satisfied else 0) * min(
            1.0, path_len_weight / float(self.steps_taken)
        )
