...
def main():
    ...
    large_objects2idx = {
        obj: i for i, obj in enumerate(constants.map_save_large_objects)
    }
    ...
    if args.use_sem_policy:
        ...
        sd = torch.load(
            f"{args.models_dir}/semantic_policy/FILM_best_model_multi.pt",
            map_location=device,
        )
    ...
    for e in range(num_scenes):
        np.random.seed(e)
        c1 = np.random.choice(local_w)
        np.random.seed(e + 1000)
        c2 = np.random.choice(local_h)
    ...
    (
        ...
        goal_success_s,
        next_step_dict_s,
    ) = envs.plan_act_and_preprocess(planner_inputs, goal_spotted_s)
    ...
    consecutive_interaction_s, target_instance_s = [None] * num_scenes, [
        None
    ] * num_scenes
    for e in range(num_scenes):
        ...
        if goal_success_s[e]:
            if list_of_actions_pointer_s[e] == len(list_of_actions_s[e]) - 1:
                ...
                if returned:
                    consecutive_interaction_s[e] = list_of_actions_s[e][
                        list_of_actions_pointer_s[e]
                    ][1]
    ...
    for step in range(args.num_training_frames // args.num_processes):
        if sum(finished) == args.num_processes:
            ...
            if args.leaderboard and args.test:
                ...
                pickle.dump(
                    actseqs,
                    open(
                        "results/leaderboard/actseqs_test_" + add_str + "_" + dn + ".p",
                        "wb",
                    ),
                )
            ...
            if x:
                ...
                if not (finished[e]):
                    ...
                    number_of_this_episode = (
                        args.from_idx + traj_number[e] * num_scenes + e
                    )
                    print(
                        "steps taken for episode# ",
                        number_of_this_episode - num_scenes,
                        " is ",
                        next_step_dict_s[e]["steps_taken"],
                    )
                    completed_episodes.append(number_of_this_episode)
                    pickle.dump(
                        completed_episodes,
                        open(
                            "results/completed_episodes_"
                            + args.eval_split
                            + str(args.from_idx)
                            + "_to_"
                            + str(args.to_idx)
                    ...
                    if args.leaderboard and args.test:
                        ...
                        pickle.dump(
                            actseqs,
                            open(
                                "results/leaderboard/actseqs_test_"
                                + add_str
                                + "_"
                                + dn
                                + ".p",
        ...
        for e in range(num_scenes):
            if args.delete_from_map_after_move_until_visible and (
                next_step_dict_s[e]["move_until_visible_cycled"]
                or next_step_dict_s[e]["delete_lamp"]
            ):
        ...
        if l_step == args.num_local_steps - 1:
            ...
            for e in range(num_scenes):
                ...
                if args.use_sem_policy:
                    ...
                    test_see = {}
                    map_reconst[4 + large_objects2idx["SinkBasin"]] = local_map[e][
                        4 + 1
                    ]
                    ...
                    for cat, catid in large_objects2idx.items():
                        if not (cat == "SinkBasin"):
                            map_reconst[4 + large_objects2idx[cat]] = local_map[e][
                                4 + start_idx
                            ]
                    ...
                    if local_map[e][0][120, 120] == 0:
                        ...
                        connected_regions = skimage.morphology.label(
                            1 - local_map[e][0].cpu().numpy(), connectivity=2
                        )
                        ...
                        dilated = skimage.morphology.binary_dilation(
                            local_map[e][0].cpu().numpy(), skimage.morphology.square(4)
                        )
                        mask = skimage.morphology.convex_hull_image(dilated).astype(
                            float
                        )
                    ...
                    if (
                        goal_name in all_objects2idx
                        and next_step_dict_s[e]["steps_taken"] >= 30
                    ):
                        ...
                        pred_probs = (
                            1 - args.explore_prob
                        ) * pred_probs + args.explore_prob * mask_grid * 1 / float(
                            where_ones
                        )

                        # Now sample from pred_probs according to goal idx
                        goal_name = list_of_actions_s[e][list_of_actions_pointer_s[e]][
                            0
                        ]
                        if goal_name == "FloorLamp":
                            pred_probs = (
                                pred_probs[all_objects2idx[goal_name]]
                                + pred_probs[all_objects2idx["DeskLamp"]]
                            )
        ...
        for e in range(num_scenes):
            ...
            if local_map[e, cn, :, :].sum() != 0.0:
                ...
                if np.sum(cat_semantic_scores) != 0:
                    ...
                    if (
                        args.delete_from_map_after_move_until_visible
                        or args.delete_pick2
                    ):
                        found_goal[e] = 0
                        goal_spotted_s[e] = False
            else:
        ...
        for e, p_input in enumerate(planner_inputs):
            ...
            if (
                not (args.no_pickup)
                and (args.map_mask_prop != 1 or args.no_pickup_update)
                and next_step_dict["picked_up"]
                and goal_success_s[e]
            ):
        ...
        for e in range(num_scenes):
            ...
            if (
                task_finish[e]
                and not (finished[e])
                and not (number_of_this_episode in skip_indices)
            ):
                f = open(
                    "results/logs/log_"
                    + args.eval_split
                ...
                if not (args.test):
                    log_entry, success = envs.evaluate(
                        e
                    ...
                    print(
                        "saving success and failures for episode # ",
                        number_of_this_episode,
                        "and process number is",
                        e,
                    )
                    pickle.dump(
                        successes,
                ...
                pickle.dump(
                    analyze_recs,
                    open(
                        "results/analyze_recs/"
                        + args.eval_split
                        + "_anaylsis_recs_from_"
                        + str(args.from_idx)
                        + "_to_"
                        + str(args.to_idx)
                        + "_"
