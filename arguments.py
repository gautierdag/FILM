...
def get_args():
    ...
    parser.add_argument(
        "-v", "--visualize", type=int, default=0, help="1:Render the frame (default: 0)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-fw", "--frame_width", type=int, default=160, help="Frame width (default:84)"
    )
    parser.add_argument(
        "-fh", "--frame_height", type=int, default=120, help="Frame height (default:84)"
    )
    parser.add_argument(
        "-el",
        "--max_episode_length",
    ...
    parser.add_argument(
        "--hfov", type=float, default=60.0, help="horizontal field of view in degrees"
    )
    parser.add_argument(
        "--randomize_env_every",
        type=int,
    ...
    parser.add_argument(
        "--global_hidden_size", type=int, default=256, help="local_hidden_size"
    )
    parser.add_argument(
    ...
    parser.add_argument("--floor_thr", type=int, default=50, help="floor_thr in cm")
    ...
    # settings
    parser.add_argument(
        "--splits", type=str, default="alfred_data_small/splits/oct21.json"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="valid_seen",
        choices=["train", "valid_seen", "valid_unseen", "tests_unseen", "tests_seen"],
    )
    ...
    parser.add_argument(
        "--learned_depth", dest="use_learned_depth", action="store_true"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--agent_height_change", type=float, default=0
    ...
    parser.add_argument(
        "--debug_max_steps", dest="debug_max_steps", action="store_true"
    )
    ...
    parser.add_argument(
        "--save_training_pictures", dest="save_training_pictures", action="store_true"
    )

    parser.add_argument("--exclude_list", type=str, default="")
    parser.add_argument(
        "--load_cuda_goals", dest="load_cuda_goals", action="store_true"
    )
    parser.add_argument(
        "--save_cuda_goals", dest="save_cuda_goals", action="store_true"
    )
    ...
    if args.cuda:
        if args.auto_gpu_config:
            ...
            if num_gpus == 1:
                ...
                args.num_processes_per_gpu = min(
