...
from alfred_utils.gen.utils.game_util import (
    get_objects_of_type,
...
class ThorEnvCode(Controller):
    ...
    def __init__(
        ...
        self.observation_space = gym.spaces.Box(
            0, 255, (3, constants.SCREEN_HEIGHT, constants.SCREEN_WIDTH), dtype="uint8"
        )
        ...
        if args.use_learned_depth:
            self.depth_gpu = torch.device(
                "cuda:" + str(args.depth_gpu) if args.cuda else "cpu"
            )
    
            # Use Valts HLSM depth
            model_path = (
                "depth/hlsm_model-2000-best_silog_10.13741"  # 45 degrees only model
            )
            self.depth_pred_model = AlfredSegmentationAndDepthModel()

            state_dict = torch.load(
                f"{args.models_dir}/{model_path}",
                map_location=self.depth_gpu,
            )["model"]
            ...
            if self.args.separate_depth_for_straight:
                model_path = "depth/hlsm_model-102500-best_silog_17.00430"
    ...
    def setup_scene(self, traj_data, task_type, r_idx, args, reward_type="dense"):
        ...
        obs, _, _, info = self.step(dict(action="LookDown_15", forceAction=True))
        if not (r_idx is None):
            print(
                "Task: %s" % (traj_data["turk_annotations"]["anns"][r_idx]["task_desc"])
            )
    ...
    def get_pose_change_approx_relative(self, last_action, whether_success):
        if not (whether_success):
            ...
            if "MoveAhead" in last_action:
                do = 0.0
                if (
                    abs(self.o + 2 * np.pi) <= 1e-1
                    or abs(self.o) <= 1e-1
                    or abs(self.o - 2 * np.pi) <= 1e-1
                ):  # o is 0
                    dx = 0.25
                    dy = 0.0
                elif (
                    abs(self.o + 2 * np.pi - np.pi / 2) <= 1e-1
                    or abs(self.o - np.pi / 2) <= 1e-1
                    or abs(self.o - 2 * np.pi - np.pi / 2) <= 1e-1
                ):
                    dx = 0.0
                    dy = 0.25
                elif (
                    abs(self.o + 2 * np.pi - np.pi) <= 1e-1
                    or abs(self.o - np.pi) <= 1e-1
                    or abs(self.o - 2 * np.pi - np.pi) <= 1e-1
                ):
                    dx = -0.25
                    dy = 0.0
                elif (
                    abs(self.o + 2 * np.pi - 3 * np.pi / 2) <= 1e-1
                    or abs(self.o - 3 * np.pi / 2) <= 1e-1
                    or abs(self.o - 2 * np.pi - 3 * np.pi / 2) <= 1e-1
                ):
                    dx = 0.0
                    dy = -0.25
                else:
    ...
    def restore_scene(self, object_poses, object_toggles, dirty_and_empty):
        ...
        if len(object_toggles) > 0:
            super().step(
                (dict(action="SetObjectToggles", objectToggles=object_toggles))
            )
    ...
    def set_task(
        ...
        self.task = get_task(
            ...
            task_type_optional=task_type_optional,
        )
        ...
        if action["action"] == "<<stop>>":
            ...
            if smooth_nav:
                ...
                if "LookUp" in action["action"]:
                    ...
                    if (
                        not (self.args.approx_last_action_success)
                        and self.event.metadata["lastActionSuccess"]
                    ):
                        self.camera_horizon += -angle
                    elif self.args.approx_last_action_success and self.success_for_look(
                        action["action"]
                    ):
                        self.camera_horizon += -angle
        ...
        for xx in np.arange(0.1, 1.0001, 0.1):
            if xx < 1:
                teleport_action = {
                    "action": "TeleportFull",
                    "rotation": np.round(
                        start_rotation * (1 - xx) + end_rotation * xx, 3
                    ),
                    "x": position["x"],
                    "z": position["z"],
                    "y": position["y"],
    ...
    def to_thor_api_exec(self, action, object_id="", smooth_nav=False):
        ...
        if "RotateLeft" in action:
            action = dict(action="RotateLeft", degrees="90", forceAction=True)
            obs, rew, done, info = self.step(action, smooth_nav=smooth_nav)
        elif "RotateRight" in action:
            action = dict(action="RotateRight", degrees="90", forceAction=True)
            obs, rew, done, info = self.step(action, smooth_nav=smooth_nav)
        elif "MoveAhead" in action:
            action = dict(action="MoveAhead", forceAction=True)
            obs, rew, done, info = self.step(action, smooth_nav=smooth_nav)
        elif "LookUp" in action:
            # if abs(self.event.metadata['agent']['cameraHorizon']-0) <5:
            if abs(self.camera_horizon - 0) < 5:
                action = dict(action="LookUp_0", forceAction=True)
            else:
                action = dict(action=action, forceAction=True)
            obs, rew, done, info = self.step(action, smooth_nav=smooth_nav)
        elif "LookDown" in action:
            # if abs(self.event.metadata['agent']['cameraHorizon'] - 90) <5:
            if abs(self.camera_horizon - 90) < 5:
                action = dict(action="LookDown_0", forceAction=True)
            else:
                action = dict(action=action, forceAction=True)
            obs, rew, done, info = self.step(action, smooth_nav=smooth_nav)
        elif "OpenObject" in action:
            action = dict(action="OpenObject", objectId=object_id, moveMagnitude=1.0)
            obs, rew, done, info = self.step(action)
        elif "CloseObject" in action:
            action = dict(action="CloseObject", objectId=object_id, forceAction=True)
            obs, rew, done, info = self.step(action)
        elif "PickupObject" in action:
            action = dict(action="PickupObject", objectId=object_id)
            obs, rew, done, info = self.step(action)
        elif "PutObject" in action:
            inventory_object_id = self.event.metadata["inventoryObjects"][0]["objectId"]
            new_inventory_object_id = self.event.metadata["inventoryObjects"][0][
                "objectId"
            ]
            action = dict(
                action="PutObject",
            ...
            action = dict(action="ToggleObjectOn", objectId=object_id)
            obs, rew, done, info = self.step(action)

        elif "ToggleObjectOff" in action:
            action = dict(action="ToggleObjectOff", objectId=object_id)
            obs, rew, done, info = self.step(action)
            ...
            action = dict(action="SliceObject", objectId=object_id)
            obs, rew, done, info = self.step(action)
        else:
            raise Exception(
    ...
    def va_interact(
        ...
        if type(interact_mask) is str and interact_mask == "NULL":
            ...
            for color_id, intersection_count in instance_counter.most_common():
                union_count = np.sum(
                    np.logical_or(
                        np.all(instance_segs == color_id, axis=2),
                        interact_mask.astype(bool),
                    )
                )
                iou_scores[color_id] = intersection_count / float(union_count)
            iou_sorted_instance_ids = list(
                OrderedDict(
                    sorted(iou_scores.items(), key=lambda x: x[1], reverse=True)
                )
            )
            ...
            all_ids = [
                color_to_object_id[color_id]
                for color_id in iou_sorted_instance_ids
            ...
            if len(instance_ids) == 0:
                if not (
                    "Rotate" in action or "MoveAhead" in action or "Look" in action
                ):
        ...
        if debug:
            print(
                "taking action: "
                + str(action)
                + " on target_instance_id "
                + str(target_instance_id)
            )
        try:
            obs, rew, done, infos, event, api_action = self.to_thor_api_exec(
