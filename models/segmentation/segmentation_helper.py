...
class SegmentationHelper:
    def __init__(self, agent):
        ...
        if args.use_sem_seg:
            self.sem_seg_gpu = torch.device(
                "cuda:" + str(args.sem_seg_gpu) if args.cuda else "cpu"
            )
            # LARGE
            self.sem_seg_model_alfw_large = load_pretrained_model(
                f"{args.models_dir}/segmentation/alfworld_receps_lr5e-3_003.pth",
                torch.device("cuda:0" if args.cuda else "cpu"),
                "recep",
            )
            self.sem_seg_model_alfw_large.eval()
            self.sem_seg_model_alfw_large.to(
                torch.device("cuda:0" if args.cuda else "cpu")
            )

            # SMALL
            self.sem_seg_model_alfw_small = load_pretrained_model(
                f"{args.models_dir}/segmentation/alfworld_objects_lr5e-3_005.pth",
                self.sem_seg_gpu,
                "obj",
            )
            ...
            self.large_idx2large_object = {
                v: k for k, v in self.large_objects2idx.items()
            }

            self.small = alfworld_constants.OBJECTS_DETECTOR
            self.small_objects2idx = {k: i for i, k in enumerate(self.small)}
            self.small_idx2small_object = {
                v: k for k, v in self.small_objects2idx.items()
            }
            ...
            v_outputs = Instances(
                ...
                scores=torch.cat(
                    [
                        ...
                        self.segmented_dict["large"]["scores"],
                    ],
                    dim=0,
                ),
                pred_classes=np.array(
                    [
                        self.small_idx2small_object[int(s.item())]
                        for s in self.segmented_dict["small"]["classes"]
                    ]
                    + [
                        self.large_idx2large_object[int(s.item())]
                        for s in self.segmented_dict["large"]["classes"]
                    ]
                ),
                pred_masks=torch.cat(
                    [
                        torch.tensor(
                            self.segmented_dict["small"]["masks"].astype(bool)
                        ),
                        torch.tensor(
                            self.segmented_dict["large"]["masks"].astype(bool)
                        ),
                    ],
                    dim=0,
                ),
    ...
    def segmentation_for_map(self):
        ...
        semantic_seg = np.zeros(
            (
                self.args.env_frame_height,
                self.args.env_frame_width,
                self.args.num_sem_categories,
            )
        )
    ...
    def get_instance_mask_seg_alfworld_both(self):
        ...
        ims = [rgb]
        im_tensors = [
            self.transform(i).to(device=self.sem_seg_gpu)
            if self.args.cuda
            else self.transform(i)
            for i in ims
        ]
        ...
        for cat_name in self.agent.total_cat2idx:
            if not (cat_name in ["None", "fake"]):
                if cat_name in self.large:
                    ...
                    if cat_name == self.agent.goal_name or (
                        cat_name in self.agent.cat_equate_dict
                        and self.agent.cat_equate_dict[cat_name] == self.agent.goal_name
                    ):
        ...
        pred_masks_small = (
            results_small["masks"][indices_small].squeeze(1).detach().cpu().numpy()
        )  # pred_masks[i] has shape (300,300)
        ...
        for ci in range(len(pred_classes_small)):
            if (
                self.small_idx2small_object[int(pred_classes_small[ci].item())]
                in self.agent.cat_equate_dict
            ):
                cat = self.small_idx2small_object[int(pred_classes_small[ci].item())]
                pred_classes_small[ci] = self.small_objects2idx[
                    self.agent.cat_equate_dict[cat]
                ]
        ...
        pred_masks_large = (
            results_large["masks"][indices_large].squeeze(1).detach().cpu().numpy()
        )  # pred_masks[i] has shape (300,300)
        if self.args.with_mask_above_05:
    ...
    def get_sem_pred(self, rgb):
        if not (self.args.use_sem_seg):
            ...
            if (
                self.agent.pointer == 2
                or (
                    self.agent.pointer == 1
                    and self.agent.last_success
                    and self.agent.last_action_ogn == "PutObject"
                )
            ) and self.agent.task_type == "pick_two_obj_and_place":
                small_len = len(self.segmented_dict["small"]["scores"])
                large_len = len(self.segmented_dict["large"]["scores"])
                ...
                self.segmented_dict["small"]["boxes"] = self.segmented_dict["small"][
                    "boxes"
                ][incl_idx]
                self.segmented_dict["small"]["classes"] = self.segmented_dict["small"][
                    "classes"
                ][incl_idx]
                self.segmented_dict["small"]["masks"] = self.segmented_dict["small"][
                    "masks"
                ][incl_idx]
                self.segmented_dict["small"]["scores"] = self.segmented_dict["small"][
                    "scores"
                ][incl_idx]
        ...
        if self.args.save_pictures:
            cv2.imwrite(
                self.agent.picture_folder_name
                + "Sem/"
                + "Sem_"
                + str(self.agent.steps_taken)
                + ".png",
                sem_vis,
            )
    ...
    def get_target(self, interact_mask, mask_px_sample=1):
        ...
        inv_obj = (
            self.agent.event.metadata["inventoryObjects"][0]["objectId"]
            if len(self.agent.event.metadata["inventoryObjects"]) > 0
            else None
        )
        all_ids = [
