    ...
    if test:
        if appended:
            if unseen:
                return pickle.load(
                    open(
                        "models/instructions_processed_LP/instruction2_params_test_unseen_appended.p",
                        "rb",
                    )
                )
            else:
                return pickle.load(
                    open(
                        "models/instructions_processed_LP/instruction2_params_test_seen_appended.p",
                        "rb",
                    )
                )
        else:
            if unseen:
                return pickle.load(
                    open(
                        "models/instructions_processed_LP/instruction2_params_test_unseen_916_noappended.p",
                        "rb",
                    )
                )
            else:
                return pickle.load(
                    open(
                        "models/instructions_processed_LP/instruction2_params_test_seen_916_noappended.p",
                        "rb",
                    )
                )
    else:
        if appended:
            if unseen:
                return pickle.load(
                    open(
                        "models/instructions_processed_LP/instruction2_params_val_unseen_appended.p",
                        "rb",
                    )
                )
            else:
                return pickle.load(
                    open(
                        "models/instructions_processed_LP/instruction2_params_val_seen_appended.p",
                        "rb",
                    )
                )
        else:
            if unseen:
                return pickle.load(
                    open(
                        "models/instructions_processed_LP/instruction2_params_val_unseen_916_noappended.p",
                        "rb",
                    )
                )
            else:
                return pickle.load(
                    open(
                        "models/instructions_processed_LP/instruction2_params_val_seen_916_noappended.p",
                        "rb",
                    )
                )
...
def determine_consecutive_interx(list_of_actions, previous_pointer, sliced=False):
    ...
    if previous_pointer <= len(list_of_actions) - 1:
        if (
            list_of_actions[previous_pointer][0]
            == list_of_actions[previous_pointer + 1][0]
        ):
            returned = True
        ...
        # Micorwave or Fridge
            ...
            list_of_actions[previous_pointer][1] == "OpenObject"
            and list_of_actions[previous_pointer + 1][1] == "PickupObject"
        ):
            returned = True
        ...
        # Micorwave or Fridge
            ...
            list_of_actions[previous_pointer][1] == "PickupObject"
            and list_of_actions[previous_pointer + 1][1] == "CloseObject"
        ):
            returned = True
            # target_instance = list_of_target_instance[-2] #e.g. Fridge
            target_instance = list_of_actions[previous_pointer - 1][0]
        # Faucet
        elif list_of_actions[previous_pointer + 1][0] == "Faucet" and list_of_actions[
            previous_pointer + 1
        ][1] in ["ToggleObjectOn", "ToggleObjectOff"]:
            returned = True
            target_instance = "Faucet"
        # Pick up after faucet
        elif (
            list_of_actions[previous_pointer][0] == "Faucet"
            and list_of_actions[previous_pointer + 1][1] == "PickupObject"
        ):
            returned = True
            # target_instance = list_of_target_instance[0]
...
def get_list_of_highlevel_actions(
    traj_data, test=False, test_dict=None, args_nonsliced=False, appended=False
):
    if not (test):
        (
            language_goal,
    ...
    if test:
        ...
        (
            ...
            parent_target,
            sliced,
        ) = get_arguments_test(test_dict, instruction)
