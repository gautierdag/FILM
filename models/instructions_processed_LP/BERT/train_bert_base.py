...
x_val_seen = [x_val_seen[i] for i in sampled_val_seen]
y_val_seen = [y_val_seen[i] for i in sampled_val_seen]
...
x_val_unseen = val_set_unseen["x_low"]
y_val_unseen = val_set_unseen["y"]
if args.no_appended:
    x_val_unseen = val_set_unseen["x"]
random.seed(1)
sampled_val_unseen = random.sample(range(len(x_val_unseen)), 100)
x_val_unseen = [x_val_unseen[i] for i in sampled_val_unseen]
y_val_unseen = [y_val_unseen[i] for i in sampled_val_unseen]
labels_val_unseen = torch.tensor(y_val_unseen).to(device)
encoding_v_us = tokenizer(
    x_val_unseen, return_tensors="pt", padding=True, truncation=True
)
...
for t in range(50):
    ...
    if t % 1 == 0:
        with torch.no_grad():
            ...
            print(
                "validation accuracy (seen): ",
                accuracy(
                    y_pred=outputs_val_seen.logits, y_batch=labels_val_seen.view(-1)
                ),
            )
            del outputs_val_seen
            outputs_val_unseen = model(
                input_ids_val_unseen, attention_mask=attention_mask_val_unseen
            )
            print(
                "validation accuracy (unseen): ",
                accuracy(
                    y_pred=outputs_val_unseen.logits, y_batch=labels_val_unseen.view(-1)
                ),
            )
