...
parser.add_argument(
    ...
    "--task",
    type=str,
    help="type in one of mrecep, object, parent, toggle, sliced",
)
parser.add_argument(
    "-v", "--verbose", type=int, default=0, help="print training output"
)
parser.add_argument(
    "-model_type",
    "--model_type",
    type=str,
    default="bert-base",
    help="one of roberta-large, roberta-base, bert-base, bert-large",
)
parser.add_argument(
    "-load",
    "--load",
...
template_by_label = pickle.load(
    open("data/alfred_data/alfred_dicts/template_by_label.p", "rb")
)
train_set = pickle.load(
    open("data/alfred_data/train_text_with_ppdl_low_appended.p", "rb")
)
val_set_seen = pickle.load(
    open("data/alfred_data/val_seen_text_with_ppdl_low_appended.p", "rb")
)
val_set_unseen = pickle.load(
    open("data/alfred_data/val_unseen_text_with_ppdl_low_appended.p", "rb")
)
...
if args.task == "mrecep":
    label_arg = "mrecep_targets"
    num_labels = len(obj2idx)
elif args.task == "object":
    label_arg = "object_targets"
    num_labels = len(obj2idx)
elif args.task == "parent":
    label_arg = "parent_targets"
    num_labels = len(recep2idx)
elif args.task == "toggle":
    label_arg = "toggle_targets"
    num_labels = len(toggle2idx)
...
x_val_seen = val_set_seen["x_low"]
y_val_seen = val_set_seen["y"]
if args.no_appended:
...
for t in range(20):
    ...
    with torch.no_grad():
        model.eval()
        outputs_val_seen = model(
            input_ids_val_seen, attention_mask=attention_mask_val_seen
        )
        val_seen_acc = accuracy(
            y_pred=outputs_val_seen.logits, y_batch=label_val_seen.view(-1)
        )
        if args.verbose:
        ...
        torch.save(
            model.state_dict(),
            super_folder + "argument_models/" + save_folder_name + model_name,
        )
...
highest_test = np.argwhere(
    accuracy_dictionary["test"] == np.amax(accuracy_dictionary["test"])
)
highest_test = highest_test.flatten().tolist()

training_acc_highest_h = np.argmax(
    [accuracy_dictionary["training"][h].detach().cpu().numpy() for h in highest_test]
)
...
if os.path.isfile(file_path):
    for CleanUp in glob.glob(
        super_folder + "argument_models/" + save_folder_name + "*.pt"
    ):
