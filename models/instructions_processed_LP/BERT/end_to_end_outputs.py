...
parser.add_argument(
    ...
    "--split",
    type=str,
    choices=["val_unseen", "val_seen", "tests_seen", "tests_unseen"],
    required=True,
)
...
def accurate_both(y_pred1, y_batch1, y_pred2, y_batch2):
    ...
    #
    num_both_accurate = torch.sum(
        (y_hat1 == y_batch1).long() * (y_hat2 == y_batch2).long()
    )
...
template_by_label = pickle.load(
    open("data/alfred_data/alfred_dicts/correct_template_by_label_ppdl.p", "rb")
)
new_labels_dict = pickle.load(
    open("data/alfred_data/alfred_dicts/correct_labels_dict_ppdl.p", "rb")
)
val_set_unseen = pickle.load(
    open("data/alfred_data/" + args.split + "_text_with_ppdl_low_appended.p", "rb")
)
...
task_to_label_mapping = {
    "mrecep_target": [1],
    "object_target": [0, 1, 2, 3, 4, 5, 6],
    "parent_target": [0, 1, 2, 3, 4, 6],
    "toggle_target": [5],
...
x_val_seen_p = [
    str(int(vs_idx2predicted_label[i])) + " " + x for i, x in enumerate(x_val_seen)
]
encoding_v_s = tokenizer(
    x_val_seen_p, return_tensors="pt", padding=True, truncation=True
)
input_ids_val_seen = encoding_v_s["input_ids"].to(device)
attention_mask_val_seen = encoding_v_s["attention_mask"].to(device)

parent_target_classifier = BertForSequenceClassification.from_pretrained(
    "bert-base-uncased", num_labels=len(recep2idx)
).to(device)
parent_target_classifier.load_state_dict(
    torch.load(os.path.join(save_folder_name, "parent.pt"))
)
parent_outputs_hat = get_prediction(
    parent_target_classifier, 9, input_ids_val_seen, attention_mask_val_seen
)
del parent_target_classifier

object_target_classifier = BertForSequenceClassification.from_pretrained(
    "bert-base-uncased", num_labels=len(obj2idx)
).to(device)
object_target_classifier.load_state_dict(
    torch.load(os.path.join(save_folder_name, "object.pt"))
)
object_outputs_hat = get_prediction(
    object_target_classifier, 9, input_ids_val_seen, attention_mask_val_seen
)
del object_target_classifier

sliced_target_classifier = BertForSequenceClassification.from_pretrained(
    "bert-base-uncased", num_labels=2
).to(device)
sliced_target_classifier.load_state_dict(
    torch.load(os.path.join(save_folder_name, "sliced.pt"))
)
sliced_outputs_hat = get_prediction(
    sliced_target_classifier, 9, input_ids_val_seen, attention_mask_val_seen
)
del sliced_target_classifier

mrecep_target_classifier = BertForSequenceClassification.from_pretrained(
    "bert-base-uncased", num_labels=len(obj2idx)
).to(device)
mrecep_target_classifier.load_state_dict(
    torch.load(os.path.join(save_folder_name, "mrecep.pt"))
)
mrecep_outputs_hat = get_prediction(
    mrecep_target_classifier, 9, input_ids_val_seen, attention_mask_val_seen
)
...
for i, instruction in enumerate(instructions):
    ...
    instruction2_params_test_unseen[instruction] = {
        "task_type": task_type,
