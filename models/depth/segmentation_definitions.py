...
_EXTRA_TOKENS = [INVENTORY_OBJECT_STR]
...
_PICKABLES = [
    s
    for s in _INTERACTIVE_OBJECTS
    if (
        (s not in _RECEPTACLE_OBJECTS)
        and (s not in _OPENABLES)
        and (s not in _TOGGLABLES)
    )
    or (s in _MOVABLE_RECEPTACLES)
]
...
OBJECT_INTID_TO_COLOR = {
    i: _compute_object_intid_to_color(i) for i in range(len(OBJECT_CLASSES))
}
