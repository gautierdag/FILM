...
class DataLoadPreprocess(Dataset):
    def __init__(self, mode="train", grid_size=8, uniform=True, mask=True):
        if mode == "train":
            lists_path = (
                "models/semantic_policy/data/maps/train/list_of_dicts_multitask.p"
            )
        ...
        self.large_objects2idx = {
            obj: i for i, obj in enumerate(map_save_large_objects)
        }
    ...
    def __getitem__(self, idx):
        ...
        map_learned = pickle.load(
            open(self.maps_dir + cur_dict["map_learned"], "rb")
        ).cpu()
        total_cat2idx_learned = pickle.load(
            open(
                self.maps_dir
                + "/".join(cur_dict["map_learned"].split("/")[:-1])
                + "/total_cat2idx.p",
                "rb",
            )
        )
        map_gt = pickle.load(open(self.maps_dir + cur_dict["map_gt"], "rb")).cpu()
        total_cat2idx_gt = pickle.load(
            open(
                self.maps_dir
                + "/".join(cur_dict["map_gt"].split("/")[:-1])
                + "/total_cat2idx.p",
                "rb",
            )
        )
        ...
        for cat, catid in total_cat2idx_learned.items():
            if cat in self.large_objects2idx:
                map_learned_reconst[4 + self.large_objects2idx[cat]] = map_learned[
                    4 + catid
                ]
        ...
        for cat, catid in total_cat2idx_gt.items():
            if cat in self.all_objects2idx:
                map_gt_reconst[self.all_objects2idx[cat]] = self.into_grid(
                    map_gt[4 + catid], map_learned_mask
                )

        return_dict = {
            "map_learned": map_learned_reconst,
            "map_gt": map_gt_reconst,
            "mask_grid": map_learned_mask,
            "map_gt_obs": map_gt[0],
        }
        return return_dict
...
def KLDivergence(input_prob, target_prob):
    each_class = target_prob * (
        torch.log(target_prob) - torch.log(input_prob)
    )  # shape is 32 x 64
            ...
            if step % args.eval_freq == 0 and step > 0:
                ...
                with torch.no_grad():
                    for e_step, eval_batched in enumerate(dataloader_eval):
                        ...
                        yg = argmax_gt - 8 * xg
                        dist += torch.sqrt(
                            torch.dot((x - xg), (x - xg).T)
                            + torch.dot((y - yg), (y - yg).T)
                        )
                        loss = 0.0
                        ...
                        if e_step % 5 == 0:
                            for k in range(73):
                                if torch.sum(eval_batched["map_gt"].squeeze()[k]) == 0:
                                    ...
                                    cur_dir = (
                                        "models/semantic_policy/data/map_vis/vis_seed"
                                        + str(args.seed)
                                        + "_lr"
                                        + str(args.lr)
                                        + "/"
                                    ...
                                    pickle.dump(
                                        ...
                                        .view((240, 240))
                                        .numpy(),
                                        open(cur_dir + "obstruction_gt.p", "wb"),
                                    )

                                    save = (
                                        eval_batched["map_learned"][0, 0]
                                        .cpu()
                                        .view((240, 240))
                                        .numpy()
                                    )
                                    ...
                                    pickle.dump(
                                        ...
                                        .view((240, 240))
                                        .numpy(),
                                        open(cur_dir + "obstruction_pred.p", "wb"),
                                    )

                                    save = cv2.resize(
                                        pred_probs[0, k].view(8, 8).cpu().numpy(),
                                        (240, 240),
                                    )
                                    save = save / np.max(save)
                                    save = (255 * save).astype(np.uint8)
                                    ...
                                    pickle.dump(
                                        pred_probs[0, k].view(8, 8).cpu().numpy(),
                                        open(cur_dir + "pred_probs.p", "wb"),
                                    )

                                    save = cv2.resize(
                                        map_gt[0, k].view(8, 8).cpu().numpy(),
                                        (240, 240),
                                    )
                                    save = save / np.max(save)
                                    save = (255 * save).astype(np.uint8)
                    ...
                    torch.save(
                        model_cop.state_dict(),
                        "map_pred_models_multi/"
                        + str(args.dn)
                        + "seed_"
                        + str(args.seed)
                        + "lr_"
